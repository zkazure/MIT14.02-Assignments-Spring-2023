...
* DONE Question 1: Economic Data [5/5]
** DONE Q 1.1 [4/4]
:PROPERTIES:
:header-args:python: :session *q11* :results output
:END:
...
** DONE Q 1.2 [3/3]
:PROPERTIES:
:header-args:python: :session *q12* :results output
:END:
...
#+begin_src python :results file graphics :exports results :file ./img/q13.png
  import matplotlib.pyplot as plt
  import matplotlib.dates as mdates
  import pandas as pd
  import numpy as np
...
** DONE Q 1.4 [3/3]
:PROPERTIES:
:header-args:python: :session *q14* :results output
:END:
...
#+begin_src python :results none
  ...
  df = pd.read_csv("./data/LNS12300060.csv",
                   parse_dates=["observation_date"],
                   index_col="observation_date")
#+end_src

*** DONE plot
Download the quarterly Employment to Population Ratio for 25-54 year old
(LNS12300060), from the earliest available date to the latest.
Plot the non-employment to population ratio,
calculated as 100% - employment to population ratio.

#+begin_src python :results file graphics :exports results :file ./img/q14.png
  plt.title("unemployment to population ratio")
  plt.ylabel("100%")
  plt.plot(df.index, df["LNS12300060"], label="non-employment-population-ratio")
  plt.legend()
...
*** DONE Q&A
How does the non-employment to population ratio in Q4 2019 compare to Q2 2020?

#+begin_src python :results output
  q4_2019 = df.loc["2019-10-01":"2019-12-01"]
  q2_2020 = df.loc["2020-04-01":"2020-06-01"]
  print(f"2019 q4: {q4_2019["LNS12300060"].mean()}")
  print(f"2020 q2: {q2_2020["LNS12300060"].mean()}")
#+end_src

#+RESULTS:
: 2019 q4: 80.33333333333333
: 2020 q2: 71.5

*** DONE Q&A
How do the changes in the non-employment to population ratio compare to the
period between Q2 2008 and Q2 2009?

#+begin_src python :results output
  q2_2008 = df.loc["2008-04-01":"2008-06-01"]
  q2_2009 = df.loc["2009-04-01":"2009-06-01"]
  print(f"2008 q2: {q2_2008["LNS12300060"].mean()}")
  print(f"2009 q2: {q2_2009["LNS12300060"].mean()}")
#+end_src

#+RESULTS:
: 2008 q2: 79.5
: 2009 q2: 76.00000000000001

** DONE Q 1.5
Compare the relationship between the evolution of the Unemployment Rate and the
evolution of the Non-Employment to Population Ratio from peak to trough of the
COVID-19 recession and the 2008 financial crisis.

Why is there not a 1-1 relationship between the two variables (write down the
formulas for each one)? What can you conclude about labor force participation
during the COVID-19 recession (feel free to use FRED to answer this question if
helpful)?


- unemployment rate: \( \frac{U}{U+E} \)
- non-employment to population rate:
  \( 1 - \frac{E}{\text{population}} = 1 - \frac{E}{U+E+N} \)


During the COVID-19 period, there is lower labor force participation.

* DONE Question 2 Measure nominal and real GDP [4/4]
** DONE Q 2.1
Compute the Nominal GDP of each year.
...
** DONE Q 2.2
Compute the real GDP for each year using 2019 as the base year.
Then compute annual RGDP growth for each year and express it as a percentage.

\begin{align*}
\text{RGDP}_{19}(19) &= 1956 \\
\text{RGDP}_{20}(19) &= 1090 \\
\text{RGDP}_{21}(19) &= 2335 \\
\end{align*}
...
\end{align*}
** DONE Q 2.3
Compute the RGDP for each year using 2020 as the base year.
Then compute the RGDP growth and express it as a percentage.
Explain why it is different from the RGDP growth computed using 2019 as the base year.
Do the same using 2021 as the base year.


\begin{align*}
\text{RGDP}_{19}(20) &= 1170 \\
\text{RGDP}_{20}(20) &= 1250 \\
\text{RGDP}_{21}(20) &= 2150 \\
\end{align*}
...
\begin{align*}
\text{RGDP}_{19}(21) &= 1605 \\
\text{RGDP}_{20}(21) &= 1000 \\
\text{RGDP}_{21}(21) &= 1975 \\
\end{align*}
...
** DONE Q 2.4
\[ g_{T-t, T} ^{\text{BEA}} = \frac{g_{T-t, T}(T-t) + g_{T-t, T}(T)}{2} \]


\[ g_{19, 20} ^{\text{BEA}} = \frac{g_{19, 20}(19) + g_{19, 20}(20)}{2} = -18.72 \% \]

\[ g_{20, 21} ^{BEA} = \frac{g_{20, 21}(20) + g_{20, 21}(21)}{2} = 70.75 \% \]
* DONE Question 3 The Goods Market [7/7]

** DONE what is \( c_{0}, c_1 \)
\( c_0 \) is what people would consume in any condition.

\( c_1 \) represents the "marginal propensity to consume".

** DONE equilibrium condition
\( Y = Z \)

** DONE draw graph to represent equilibrium
#+begin_src python :results file graphics :exports results :file ./img/q33.png
  import matplotlib.pyplot as plt
  import numpy as np

  zx = np.linspace(0, 5, 5)
  zy = zx
  ...
  plt.plot(zx, yy)
  plt.xlabel("Income, Y")
  plt.ylabel("Demand Z, Production Y")
...
** DONE autonomous spending
\[ \frac{\mathrm{d} Y}{\mathrm{d} c_{0}} = \frac{1}{1-c_{1}} \]
** DONE Saving formula
\[ S = C +I+G -T-C = I + G - T \]
** DONE Saving and Government
Since I and T are exogenous, S increases by the same amount of G
